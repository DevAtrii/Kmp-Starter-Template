...
class ExcludeCommand : CliktCommand(name = "exclude") {

    override fun help(context: Context) = "Exclude a starter module from an existing project"
    ...
    private val moduleOption: StarterModules? by option(
        "--module",
        help = "Starter module to exclude",
    ...
    private val target: String by option("--target", help = "Gradle module to remove the dependency from")
        .default("composeApp")

    override fun run() {
        runBlocking {
            val viewModel = KoinPlatform.getKoin().get<ExcludeViewModel>()

            val module = moduleOption ?: run {
                echo("Exclude a starter module from an existing project")
                echo("")
            ...
            viewModel.exclude(
                ...
                echo("Excluded module '$moduleId' from '$dir' (mode=$resolvedMode, target=$target)")
            }.onFailure { error ->
                throw CliktError(error.message ?: "Failed to exclude module.")
            }
        }
    }
}
